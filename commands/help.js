...
module.exports.config = {
  ...
  example: "mb!help",
  description:
    "Help menu for all prefix commands, command cooldown: 2.5 seconds",
};
...
module.exports.run = async (client, message, args) => {
  ...
  try {
    ...
    if (
      ...
      client.commands.has(
        client.commands.get(client.aliases.get(args[0]).config.name)
      )
    ) {
      return message
        .reply(
...
${pu.config.name ? `**Name:** ${pu.config.name}` : ""}${
            pu.config.description ? "\n" : ""
          }${
            pu.config.description
              ? `**Description:** ${pu.config.description}`
              : ""
          }${pu.config.aliases ? "\n" : ""}${
            pu.config.aliases
