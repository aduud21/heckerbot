...
module.exports.config = {
  ...
  description: "Clear all the data that bot has collected from your server, server command cooldown: 60 seconds",
  group: 'config',
  guarded: true,
}
...
module.exports.run = async (client, message, args) => {
  ...
  try {
    let msg = await message.reply(`${client.pending} Checking database (0%)`).catch(() => {})
    ...
    if (!filelog[message.guild.id]) {
      msg.edit(`${client.fail} The bot hasn't collected any data about your server yet`).catch(() => {})
      return;
    }
    ...
    fs.writeFile('./database/realmodlogs.txt', encryptedData, (err) => {
      if (err) {
        console.error(`Error writing to modlogs file: ${err}`)
              msg.edit(`${client.fail} An error occurred internally in this bot, if this happens too much please let the owner of this bot know`).catch(() => {})
      }
    })
    msg.edit(`${client.success} Deleted all the data that the bot has collected from your server. If you do not like the bot please tell me why VIA the support server`).catch(() => {})
  } catch (err) {
