...
module.exports = async (client, message) => {
  ...
  const prefixRegex = new RegExp(
    `^(<@!?${client.user.id}>|${escapeRegex(mainPrefix)})\\s*`
  );
  ...
  var command =
    client.commands.get(cmm) || client.commands.get(client.aliases.get(cmm));
  if (!command) return;
  ...
  if (command.config.ownerOnly) {
    if (command.config.ownerOnly === true) {
      ...
      message.react("❌").catch(() => {}),
        message
          .reply(`❌-> Error 403, only owner(s) can use this command`)
          .catch(() => {});
    }
  }
  if (command.config.botperms) {
    ...
    command.config.botperms.forEach((p) => {
      if (!message.guild.members.me.permissions.has(p))
        neededPerms.push("`" + p + "`");
    });
    if (neededPerms.length) {
  ...
  if (commandFile) {
    if (client.cooldown.has(message.author.id)) {
      const remainingCooldownRL =
        UserPreventRL.get(message.author.id) - Date.now();
      if (remainingCooldownRL > 0) {
        return;
      }
      message
        .reply("Hold up there, you using prefix commands too quick!")
        .catch(() => {});
