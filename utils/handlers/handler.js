...
if (message.channel.type === 'dm') return;
  ...
  return;
    ...
    const cmm = args.shift().toLocaleLowerCase();
        ...
        if (command.config.ownerOnly) {
            if (command.config.ownerOnly === true) {
              ...
              message.react('❌'), message.reply(`❌-> Error 403, only owner(s) can use this command`)
            }
        }
        ...
        if (command.config.botperms) {
            ...
            if (neededPerms.length) {
                    ...
                    return await message.channel.send(`❌-> I am missing ${neededPerms.join(', ')} permission.`);
    ...
    if (commandFile) {
        ...
        'USE_EXTERNAL_EMOJIS',
        'ADD_REACTIONS')){ 
          message.reply('I need \`VIEW_AUDIT_LOG, MANAGE_GUILD, MANAGE_ROLES, MANAGE_CHANNELS, KICK_MEMBERS, BAN_MEMBERS CHANGE_NICKNAME, MANAGE_NICKNAMES, MANAGE_EMOJIS, VIEW_CHANNEL, SEND_MESSAGES, MANAGE_MESSAGES, EMBED_LINKS, ATTACH_FILES, USE_EXTERNAL_EMOJIS, ADD_REACTIONS\` permissions for me to work with all commands or in short i need `ADMINISTRATOR`') 
