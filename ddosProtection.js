...
async function getIpAddress(req) {
  const ip =
    (req.headers["x-forwarded-for"] || "").replace(/:\d+$/, "") ||
...
module.exports = async (request) => {
  try {
    const ipAddress =
      request.headers["cf-connecting-ip"] || (await getIpAddress(request));
    const geo =
      request.headers["cf-ipcountry"] || (await getGeoData(ipAddress));
